...
export const CoursesSection = () => {
  ...
  return (
    <div className='py-16 md:px-40 px-8'>
      ...
      <div style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }} className='grid px-4 md:px-0 md:my-16 my-10 gap-4'>
        {
        allCourses.slice(0,4).map((course, index) => <CourseCard key={index} course={course} />)
      }
      </div>

      <Link to={'/course-list'} onClick={() => scrollTo(0,0)} className='text-gray-500 border border-gray-500 px-10 py-3 rounded'>Show more</Link>
    </div>
  )
}
